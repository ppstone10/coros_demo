...
- `BodyManagement.weightHistoryKg` 按确认编辑的先后顺序保存全部体重值；数组最后一项必须与 `weightKg` 当前值一致。
- 旧快照缺少历史数组时，以既有 `weightKg` 作为历史首项迁移；不得排序、去重或删除重复值。
- 用户每次在体型管理卡确认体重后追加一项并持久化完整健康快照；场景刷新不得覆盖已存在的用户体重历史。
...
### `HLTH-VIS-030`：三端 HRV 与静息心率指针与指标线同层叠放

- Given：HRV 评估或静息心率卡具有合法范围和当前值
- When：Android Compose、iOS SwiftUI 或 HarmonyOS ArkUI 绘制右侧范围指标
- Then：三角指针与彩色指标线位于同一个绘图区，三角尖端统一朝上，底边向下跨过指标线并与横条重叠；横向位置继续由当前值在范围中的夹紧比例决定
...
- Then：卡路里弧在独立 116×116 居中容器内绘制；步数、卡路里、运动时间图标分别以 `#00DF7B`、`#FFC928`、`#D72BCC` 模板着色；HRV/静息心率的白色指针与指标线同层叠放且主体位于线上方
...
### `HLTH-VIS-040`：14 张卡片解耦为 12 类顶层样式

- Given：健康首页共有 14 张卡片，现有三端按 10 个顶层视觉组件分发，其中体力恢复与能力卡共用 Gauge，静息心率与 HRV 共用 Range
- When：三端渲染 Figma 2031 节点 `16:8096` 对应卡片
- Then：跑步能力与骑行能力继续共用能力样式；心率与压力继续共用趋势样式；其余 10 张卡片各自拥有按稳定 `HealthCardType`/卡片 ID 分发的独立顶层样式入口，总计 12 类顶层卡片样式
- And：体力恢复、跑步/骑行能力、静息心率、HRV 的左侧主值与说明、右侧概览图分别在各自组件内对齐；体力恢复使用 114×78 安全区，能力使用 121×71 安全区，静息心率与 HRV 分别使用 130 宽安全区
- And：公共外壳的 16 页面边距、8 圆角、标题行、14 上下内边距、空态、点击行为、共享数据契约以及其他 10 类卡片内容保持不变
- 异常/边界：可以复用数值文本、单位文本、路径数学等叶级 helper，但独立顶层样式不得再通过卡片类型条件分支切换为另一张不可复用卡片

### `HLTH-VIS-041`：HRV 与静息心率范围信息紧凑叠放

- Given：HRV 或静息心率卡具有合法范围数据
- When：三端渲染左侧文本与右侧范围图
- Then：HRV 左侧第一行显示本地化状态（如“偏低”“正常”），第二行继续显示平均 HRV；右侧正常范围说明移动到范围条下方，旧的第三行状态说明不再重复显示
- And：HRV 与静息心率的当前值指针和横向状态条在同一个绘图区叠放，指针位于横条上方并指向当前位置
- And：静息心率的 30/80 端点贴近横条；“30 天平均”改为带前导竖虚线语义的“近 30 天平均”，横条上以竖向虚线标出平均静息心率
- 异常/边界：当前值和平均值均须夹紧在 30–80 绘制范围内；平均值缺失时不绘制竖虚线

### `HLTH-VIS-042`：体重编辑历史按确认顺序完整持久化

- Given：用户健康快照已有当前体重或历史体重
- When：用户依次确认 61.0、62.0、60.0、59.0 kg
- Then：在既有首项之后依次追加上述四项，当前体重为 59.0；持久化 JSON 往返后顺序和重复值完全不变
- And：场景刷新保留该用户已有历史与当前体重，不用场景模板覆盖
- 异常/边界：仅接受 30.0–200.0 kg 且精确到 0.1 kg 的值；持久化失败时保持旧 UI 状态并输出既有错误 Effect

### `HLTH-VIS-043`：三端体型管理卡提供体重滑轮编辑入口

- Given：体型管理卡为有数据状态
- When：用户点击体重数值或其后的编辑图标
- Then：Android、iOS、HarmonyOS 均打开与资料完善页一致的 30.0–200.0 kg、0.1 kg 步进选择滑轮，并以当前体重作为初始值
- When：用户确认选择
- Then：通过 common `HealthAction` 更新并持久化体重历史，卡片立即显示新值；取消不产生历史记录
- And：编辑图标复用个人资料用户名后的 `icon_edit` 语义资产；人体图下方说明显示“本周主要锻炼部位”
- 异常/边界：点击卡片其他区域继续进入详情；空态不提供编辑入口

### `HLTH-VIS-044`：HRV 短状态与范围指针方向统一

- Given：HRV 卡具有当前值、正常下限与正常上限
- When：生成 HRV 左栏状态并绘制 HRV/静息心率范围指针
- Then：HRV 状态只使用“很低”“偏低”“正常”“偏高”四档短标签；状态边界直接取 common 输出的四段范围，不得由平台或 UI 根据像素宽度反推
- And：状态使用与其他卡片主值相同的白色高亮和 32sp/pt/vp 主值字号，状态与“平均 … 毫秒”之间保留 4dp/pt/vp 间隙
- And：Android、iOS 指针改为与已验收 HarmonyOS 一致的尖端朝上形状，三角底边跨过横条
- 异常/边界：缺少正常上下限时由 common 使用默认 `47–57`，非法边界在 `42–65` 内归一化；状态键由 common 输出，三端不得自行根据中文文案判断档位

### `HLTH-VIS-045`：HRV 指针与静息心率采用相同的横条重叠几何

- Given：HRV 评估和静息心率卡均具有合法范围数据
- When：三端绘制右侧横条和当前值三角指针
- Then：两张卡采用相同的纵向关系：三角尖端位于横条上方、三角底边进入横条绘制区域并与横条重叠，不得让整个三角悬浮在横条上方或仅与横条边缘相接
- And：以 18dp/pt/vp 绘图区为基准时，三角占据 `y=2..14`、横条占据 `y=10..14`；HarmonyOS 使用自身 8×7 vp Path 时，三角占据 `y=7..14`、横条占据 `y=10..14`
- And：HarmonyOS 必须显式解析 `health_visual_normal_range_short`，右侧横条下方显示格式化后的正常范围，不得回退为“健康数据暂时不可用”
- 异常/边界：正常上下限或单位缺失时显示既有 `--`/空单位回退，但不得泄漏资源键或通用不可用文案

### `HLTH-VIS-046`：HRV 四段横条与指针按真实数值范围绘制

- Given：正常 HRV 范围为总区间 `40–65`，四档依次为严重偏低 `40–42`、偏低 `42–47`、正常 `47–57`、偏高 `57–65`
- When：common 生成 HRV 可视化范围数据
- Then：输出四个具有明确 `minimum/maximum/level` 的连续分段；对应跨度为 `2/5/10/8`，三端不得再使用四等分或 `18%/20%/38%/剩余`、`23/26/49/剩余` 等固定像素比例
- And：三端横条中每段宽度等于该段跨度除以总跨度；正常数据下宽度比例依次为 `8%/20%/40%/32%`
- And：指针横向位置等于 `(current - 40) / (65 - 40)` 并夹紧到 `0...1`；例如 `current=48` 时指针位于横条宽度的 `32%`
- And：异常场景若 common 提供不同的正常上下限，四段仍按实际起止值重新计算宽度，平台只负责把 common 分段映射为颜色和像素
- 异常/边界：低于 40 或高于 65 的值分别夹紧在横条两端；非法或逆序边界由 common 归一化为连续非负分段，不得由三端各自修补业务阈值
...
| `HLTH-VIS-030` | Android 单测 + 三端结构门禁与截图/预览核对 | HRV/静息心率三角指针与横条同层叠放，尖端朝上且底边跨过线条 |
...
| `HLTH-VIS-040` | `tools/check-health-card-style-decoupling.sh` + 三端构建 + 卡片截图 | 三端均有 12 类顶层样式入口；仅能力双卡与趋势双卡复用，四张解耦卡内容对齐且外壳和其他卡片无变化 |
| `HLTH-VIS-041` | 三端结构门禁 + 三端构建 + 卡片截图 | HRV 状态/平均值与右侧范围说明不重复；两张卡的指针、平均虚线和端点紧凑对齐 |
| `HLTH-VIS-042` | common Store/JSON 往返测试 | 连续编辑顺序、重复值、旧快照迁移、场景刷新与失败回滚符合契约 |
| `HLTH-VIS-043` | 三端结构门禁 + 三端构建 + 人工点击验收 | 数值/编辑图标打开滑轮，确认刷新卡片，取消不写历史，其他卡片区域仍进详情 |
| `HLTH-VIS-044` | common 状态分档测试 + Android 指针几何测试 + 三端构建 + 用户截图 | HRV 仅显示四档短标签并按主值高亮；三端指针均尖端朝上、底边跨过横条 |
| `HLTH-VIS-045` | `tools/check-health-range-indicator-parity.sh` + 三端构建 + HRV/静息心率并排截图 | 两张卡指针底边均与横条重叠；HarmonyOS 正常范围文案完成显式解析 |
| `HLTH-VIS-046` | `HealthDashboardUseCaseTest.hrvRangeSegmentsAndPointerUseActualValues` + `tools/check-health-hrv-segments-and-harmony-save.sh` + 三端构建/截图 | common 输出 40/42/47/57/65 连续边界；三端按 8%/20%/40%/32% 绘制且 48 指向 32% |
...
- [x] 14 张卡片按 12 类顶层样式分发，仅跑步/骑行能力与心率/压力保留复用。
