...
注册登录模块只允许使用本地 mock 账号数据或本仓库内 `mock-server/` 提供的 mock 接口（接口契约见 `spec/mock-server-api-spec.md`），不接入任何真实服务器、测试服务器、内测服务器或外部认证服务。
...
- 所有账号、验证码、会话、错误态都必须由本地 mock 数据源或本仓库 `mock-server/` 生成；禁止使用真实外部服务。
...
### `AUTH-ACCOUNT-DELETE-001`：注销账户同步删除该用户健康数据

- Given：当前账号已登录，且本地存在该 `userId` 的认证记录和健康快照
- When：用户确认注销账户
- Then：删除认证账号、会话以及该 `userId` 对应的健康快照和进程内健康草稿
- And：其他账号的认证记录与健康快照保持不变
- 异常/边界：健康快照清理失败时不得报告注销成功；退出登录仍只清理会话，不删除账号或健康快照

### `AUTH-PROFILE-DEFAULT-001`：首次资料完善使用登录账号预填

- Given：注册或首次登录后的账号资料尚未完善
- When：三端打开个人信息完善页
- Then：用户名默认为固定文案 `COROS user`
- And：手机号账号只预填手机号，邮箱账号只预填邮箱，另一项保持为空
- And：已有 profile 字段优先于账号推导默认值，避免重新进入页面时覆盖用户已保存资料
- 异常/边界：无法识别为合法手机号或邮箱的本地 mock 账号不填入两者；邮箱为可选资料字段并遵循 protobuf/JSON 字段契约

### `AUTH-PROFILE-FOCUS-001`：平台表单离开输入控件后结束编辑

- Given：Android 或 HarmonyOS 的资料输入框正在编辑且键盘可见
- When：用户点击滚轮选择器、资料页其他非输入操作或关闭/确认选择面板
- Then：Android 清除输入焦点并移除红色聚焦高光；HarmonyOS 清除输入焦点并收起软键盘
- 异常/边界：点击另一个输入框应把焦点交给新输入框，不得被页面级手势立即清除
...
- 注销账户后该用户健康快照不可恢复，其他用户快照不受影响。
- 首次资料完善页按账号类型预填手机号或邮箱，用户名为 `COROS user`。
- Android/HarmonyOS 从输入框转到选择器或其他非输入操作时不保留焦点高光/软键盘。
...
- 会话失效后访问业务 mock 数据源。
- 本地登录态保存和恢复。
- 注销账号联动清理该用户健康快照并保留其他用户数据。
- 手机号/邮箱账号的资料完善默认值与 profile 邮箱字段持久化往返。
