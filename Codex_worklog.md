...
2026-07-13 会话 TTL、验证码时效与鸿蒙隐私弹窗视觉修复

## 采纳内容

- 采纳共享层统一验证码时钟：验证码发送时按共享层当前时间记录 60 秒过期时间；Android、iOS、HarmonyOS 均通过各自桥接层同步当前时间并读取同一剩余秒数，避免各端倒计时与实际校验时间不一致。
- 采纳“仅在冷启动恢复时校验会话 TTL”的会话语义：应用进入后台时才为已登录会话写入过期时间；直接从后台恢复不触发失效跳转；进程被清理后再次启动时恢复会话并判断是否过期，过期则清理会话并引导重新登录。
- 会话 TTL 配置融入共享仓储实现，保留 `LocalMockAuthRepository.SessionTtlMs` 作为唯一可直接调整的位置，未新增独立配置文件或 Debug 页面。
- 三端已接入该生命周期策略：Android 在 `ON_STOP` 暂停会话、冷启动时恢复；iOS 在 `scenePhase == .background` 暂停、创建共享适配器时恢复；HarmonyOS 在 `EntryAbility.onBackground` 暂停并持久化、恢复快照时校验。
- 采纳鸿蒙端隐私协议弹窗使用 ArkUI 分层覆盖实现：页面底层为全屏点击拦截遮罩层，协议面板作为上层底部组件，避免遮罩与面板处于同一 Column 布局而无法形成覆盖关系。
- 修正鸿蒙 ArkUI 颜色透明度格式：遮罩色由错误的 `#000000C7` 改为 `#C7000000`（`#AARRGGBB`），使底层登录页面获得与 Android/iOS 接近的暗化效果。
- 已分析注册验证码跨“设置密码”页仍继续计时的原因：共享层在最终注册时会再次校验原验证码有效期；本轮按要求未修改该规则。

## 人工审查点

- 会话 TTL 的产品语义需人工确认：当前选择“后台仅开始计时、仅冷启动判定”，与常见的“每次回到前台立即判定”不同；需确认该行为符合实际安全策略与用户预期。
- 需人工确认三端真机/模拟器的冷启动边界：不同系统对“清理后台”“系统回收进程”和“重新打开应用”的定义不同，应分别验证是否均触发重新创建应用进程与会话恢复校验。
- 注册验证码策略需产品确认：当前验证码在验证码页校验成功后，设置密码期间仍可能过期并导致注册失败。若期望验证码通过后允许用户从容设置密码，应改为签发绑定账号、一次性、短时有效的“注册验证通过凭据”；该业务规则本轮未改。
- 鸿蒙隐私弹窗的遮罩深度、圆角、底部安全区和点击拦截效果需人工视觉验收；ArkUI 设备型号、深浅色系统主题和系统字体差异可能影响最终观感。

## 验证结果

- Android/KMP 验证：执行 `./gradlew :common:check :androidApp:compileDebugKotlin`，结果 `BUILD SUCCESSFUL`。
- iOS 验证：执行 Xcode `IOSDemo` 模拟器构建，结果 `BUILD SUCCESSFUL`；会话生命周期与共享适配器改动通过编译。
- Harmony KMP bridge 验证：执行 `cd harmony-kmp-bridge && ./gradlew ohosArm64Binaries`，结果 `BUILD SUCCESSFUL`。
- HarmonyOS 应用构建验证：使用 DevEco/Hvigor 环境执行 `assembleApp --no-daemon`，结果 `BUILD SUCCESSFUL`。构建输出仍包含既有资源重名、KNOI 库校验、路由 API 弃用和未配置签名等警告，未阻断构建。
- 静态检查：本轮修改后执行过 `git diff --check`，未发现空白字符错误。

## 人工修正点

- 注册验证码在设置密码页继续计时的问题按当前决定暂不修改；如产品确认需优化，应在共享层新增“验证码已验证”的一次性注册凭据，而不能只在某一端隐藏倒计时。
- 请在 Android、iOS、HarmonyOS 分别手动验证：登录后切后台超过 TTL 直接返回不退出；清理后台后超过 TTL 再启动出现重新登录引导；未超时冷启动可正常恢复。
- 请在鸿蒙真机或目标模拟器手动验证隐私协议弹窗：背景应明显暗化、底部面板应保持正常亮度、点击遮罩区域不应穿透触发登录页控件。

---

2026-07-14 脱敏健康首页制作、资源维护敛收、界面布局调整

## 采纳内容

- 完成 Android 脱敏健康首页：聚合今日运动、睡眠、训练负荷、体力恢复及计划、能力、心率、压力、HRV、健康快测、体型管理等卡片，并按风险、正常、空状态及业务优先级生成展示顺序；卡片可点击进入“功能后续实现”占位页。
- 首页按目标稿统一为黑色运动健康风格，接入日历、手表、步数、热量、运动时长及各健康卡片资源；底部建立“体能、记录、探索、我”四项导航和选中/未选中状态，“体能”承载健康首页，“我”保留资料、退出登录和注销账户能力。
- 编辑卡片作为独立界面实现：支持添加、删除、恢复默认、保存、至少保留三项提示和长按拖拽排序；增删及顺序按登录用户持久化，健康数据协议与认证协议保持独立，但通过 `LoginStore`、认证 `userId` 和状态数据源完成统一访问及账号隔离。
- 修复首行拖拽交互：限制拖动卡片在列表可视区域内，交换首项时保持滚动锚点和卡片视觉位置，修正上下边缘自动滚动方向，只对被挤开的卡片执行让位动画，避免拖入首行后越界、吸附、消失或松手跳动。
- 完善个人资料流程：调整资料完善页尺寸与样式；新增“我”页面和个人资料修改页；用户名默认只读、点击编辑按钮后才可修改；保存按钮按实际差异决定是否启用；头像和资料修改结果持久化并同步展示；相关新页面补充 Compose Preview。
- 表单滚轮和隐私政策弹层采用同页模态遮罩，弹出期间拦截底层输入和点击，同时保留原有底部面板交互形态，没有改成独立全屏 Dialog。
- 完成资源维护收敛：清理确认未使用的 `res` 资源，并通过 `AppImages`、`AppText`、`AppColors` 等统一管理图片、文案和颜色引用，在不改变既有页面布局的前提下降低散落 `painterResource` 和硬编码内容的维护成本。

## 人工审查点

- 本轮涉及的健康首页视觉、卡片增删与拖拽、最低三项限制、弹层点击拦截、个人资料及头像持久化等交互，均已由用户完成整体验收并确认没有问题；暂无待确认的人工审查点。

## 验证结果

- 自动化验证：执行 `git diff --check && ./gradlew :common:testAndroidHostTest :androidApp:lintDebug :androidApp:assembleDebug`，单元测试、Android Lint、Debug APK 构建及差异格式检查全部通过，结果为 `BUILD SUCCESSFUL`。
- 模拟器交互验证：实际将第三张卡片长按拖至首行，拖动卡片始终留在可视区域，原首行和第二行依次下移，松手后顺序正确且列表恢复稳定，验证通过。
- 人工验收：用户已完成本轮所有功能、视觉和交互问题的最终测试，并明确确认当前实现无问题，验收通过。
