## SDD 记录规范（2026-07-17 起）：
> - 每条「采纳内容 / 人工审查点 / 验证结果 / 人工修正点」必须引用 Spec 章节号，格式如 `[auth-mock-spec §8]` 或 `[health-dashboard-cards 测试要求]`
> - Spec 文件位于 `spec/`，追溯映射见 `spec/TRACE.md`
> - 新增功能先写 Spec → 预留 TRACE.md 映射行 → 写代码 → 更新 TRACE.md
---
