...
# 2026-07-27 17:40 — iOS 头像持久化从文件系统统一到 UserDefaults

## 采纳内容
- [auth-mock-spec §12] 2026-07-27：iOS `ProfileImageStore` 头像图片二进制数据从 `Application Support/` 文件系统改为 `UserDefaults` 存储，与账号/健康卡片数据使用相同的持久化机制。`ProfileImageStore.save()` 返回 `UserDefaults` key 而非文件路径，`ProfileImageStore.image(at:)` 新增 UserDefaults key 前缀判断，兼容旧文件路径格式。
- [auth-mock-spec §12] 2026-07-27：`spec/auth-mock-spec.md` 新增 iOS 头像持久化约定段落，明确头像图片二进制数据不得单独使用文件系统路径存储。

## 人工审查点
- [auth-mock-spec §12] 旧文件路径格式的兼容代码（`UIImage(contentsOfFile:)` 回退路径）在旧数据全部被新格式覆盖后可删除；当前保留以支持过渡期间可能存在的旧文件路径 URI。
- [auth-mock-spec §12] 此改为纯 iOS 平台变更，不影响 Android/HarmonyOS 的头像处理逻辑。

## 验证结果
- `./gradlew :common:check` — 全部测试通过
- `./tools/build-shared-xcframework.sh` — iOS KMP framework 编译成功
- `./tools/check-sdd.sh` — SDD 框架校验通过

## 人工修正点
- 无

## 下轮交接
- **已完成**：iOS 头像持久化从文件系统统一到 UserDefaults（`ProfileImageStore` 重构 + spec/TRACE 更新）
- **未完成 / 阻塞项**：无
- **下轮起步建议**：无

# 2026-07-27 11:10 — 会话生命周期与健康 Mock 契约三端收口

## 采纳内容

- [AUTH-SESSION-001/002] 将会话恢复明确拆分为冷启动恢复与同进程前台恢复：后台时记录单一 TTL 截止时间，暖恢复保留有效会话并清除截止时间，进程被清理后的冷启动才依据持久化截止时间判断失效；未增加第二套 TTL。
- [HLTH-EMPTY-001] 明确健康首页空态引导由各业务卡片自身的空态摘要和操作提示承担，不新增独立引导卡；三端补齐活动、睡眠、训练负荷的可执行引导文案。
- [HLTH-CONTRACT-001/002] 在 `health_dashboard_mock.proto` 中补充稳定的场景枚举和错误契约；兼容读取旧场景名称，并由 common 统一向 Android、iOS、HarmonyOS 输出场景目录。
- [HLTH-SCENARIO-001] 三端场景选择器改为消费 common 目录，移除平台侧各自维护的场景清单。
- 补充登录生命周期、健康 Proto 往返、旧快照迁移与空态引导契约测试；健康 Mock schema 版本升级为 5。
- 修正资源门禁对 HarmonyOS “页面通过组件回调打开语言选择器”结构的误判，使其同时识别直接入口和 `onLanguageClick` 委托入口。

## 人工审查点

- 按本轮约定，W2 验收记录保持未处理，不补写或改写。
- 按本轮约定，不新增三端人工验收证据；三端现有对应功能可正常使用，小瑕疵留待后续独立处理。
- TTL 的持久化起点仍是最后一次进入后台的时刻。移动端无法可靠获得“进程被系统清理”的精确时刻，因此该方案是在不引入第二套 TTL 的前提下保持确定性与可测试性的折中。
- 本轮未触碰工作区中既有的 iOS 头像持久化修改及两个 `ScrollViewPanObserver` 暂存/删除状态文件。

## 验证结果

- 红灯验证：首次运行 `./gradlew :common:testAndroidHostTest` 因新增冷/暖恢复及健康契约 API 尚未实现而编译失败，确认测试能够捕获缺失行为。
- `./gradlew :common:testAndroidHostTest` — 通过。
- `./gradlew :common:check` — 98 条 common 测试通过。
- `./gradlew :androidApp:assembleDebug` — Android 构建通过。
- `./tools/build-shared-xcframework.sh` — iOS KMP framework 构建通过。
- `xcodebuild -project iosApp/iosApp.xcodeproj -scheme IOSDemo -sdk iphonesimulator -configuration Debug build CODE_SIGNING_ALLOWED=NO` — iOS 模拟器构建通过。
- `./tools/build-shared-harmony.sh` — common、KNOI bridge、provider 生成及 HarmonyOS `assembleApp` 全部通过。
- `./tools/check-sdd.sh`、`./tools/check-resources.sh`、`git diff --check` — 通过。
- `./tools/check-docs.sh` — 未通过，仅报告本轮前已存在的 `docs/ios_harmonyos_app_resource_management_guide.md` 冗余文档治理项。
- `./tools/check-resource-maintainability.sh` — 未通过，仅报告本轮前已存在的 7 处 iOS Preview 中文字面量。

## 人工修正点

- `docs/ios_harmonyos_app_resource_management_guide.md` 的去留需要先按文档治理规则建立迁移映射并确认正文已保留，本轮未擅自删除。
- 7 处 iOS Preview 中文字面量属于既有资源维护债务，可在后续小瑕疵清理中统一处理。
- W2 验收记录与新增三端人工验收证据均按用户要求保持未处理。

# 2026-07-27 11:34 — 修复 Android 冷暖会话恢复竞态

## 采纳内容

- [AUTH-SESSION-003] 新增 Android `SessionLifecycleCoordinator`，每个新协调器第一次收到 `ON_START` 时只执行冷启动恢复，后续 `ON_START` 才执行同进程暖恢复。
- [AUTH-SESSION-003] `LoginViewModel` 收敛为单一 `onAppStarted()` 入口，`AuthNavGraph` 移除独立冷启动 `LaunchedEffect`，避免生命周期观察器补发 `ON_START` 时暖恢复抢先清除 TTL。
- [AUTH-SESSION-003] 新增 2 条 Android JVM 单元测试，覆盖首次冷恢复、后续暖恢复，以及 Activity/协调器重建后重新执行冷恢复。
- common 的单 TTL、iOS adapter 冷恢复和 HarmonyOS 持久化冷恢复行为保持不变；三端继续遵循“新入口冷恢复、同实例前台返回暖恢复”的同一语义。

## 人工审查点

- Android 从最近任务清除后即使进程仍被系统缓存，新 Activity 会创建新的协调器，因此首次 `ON_START` 仍校验持久化 TTL，不依赖 PID 是否变化。
- 本轮只修复 Android 生命周期调度竞态，没有增加第二套 TTL，也没有修改 iOS/HarmonyOS 已通过人工场景验证的生命周期接线。
- W2 验收记录和新增三端人工验收证据继续按既定要求保持未处理。

## 验证结果

- 红灯验证：首次执行 `./gradlew :androidApp:testDebugUnitTest --tests com.example.demo.login.SessionLifecycleCoordinatorTest`，因 `SessionLifecycleCoordinator` 尚未实现而编译失败。
- 定向绿灯：同一测试命令在实现后通过。
- `./gradlew :androidApp:testDebugUnitTest` — Android JVM 单元测试通过。
- `./gradlew :common:check` — common 检查通过。
- `./gradlew :androidApp:assembleDebug` — Android Debug APK 构建通过。

## 人工修正点

- 建议在模拟器或真机复验：登录后进入后台并从最近任务清除，等待超过 10 秒后重新打开，应显示会话失效；同一实例仅后台停留超过 10 秒后直接返回，仍应保持登录。
