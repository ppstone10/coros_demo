...
KMP `common` 共享业务层的认证模块，包含两个测试类：

| 测试类 | 文件 | 测试数 |
| --- | --- | --- |
| `LoginRulesTest` | `common/src/commonTest/.../LoginRulesTest.kt` | 6 |
| `LoginUseCaseTest` | `common/src/commonTest/.../LoginUseCaseTest.kt` | 24 |
| **合计** | | **30** |
...
### LoginUseCaseTest（24 条）

| 测试 | 验证内容 | 异常态 |
| --- | --- | --- |
| `registerSuccessSavesSession` | 注册成功，session 自动保存 | — |
...
| `localSessionCanBeRestored` | 重新构造 repository 后 session 可恢复 | — |
| `mockStoreJsonPreservesProfileDisplayNameAndSession` | JSON 序列化/反序列化 round-trip | — |
